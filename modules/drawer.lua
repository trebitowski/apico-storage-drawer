...
function define_drawer()
    ...
    local define_recipe = api_define_recipe("crafting", FULL_DRAWER_ID, recipe,
                                            1)
    api_define_workbench("Apifactory", {t1 = "Items"})
...
function drawer_menu_draw(menu_id)
    ...
    if highlighted == menu_id then
        api_draw_sprite(dr_title_sprite, 0, 2 + menu.x - cam.x,
                        2 + menu.y - cam.y)
    end
end
...
function init_drawer()
    ...
    dr_title_sprite = api_define_sprite(MOD_NAME .. "_drawer_title",
                                        "sprites/drawer/title.png", 1)
